...
# Hook function to get the gradient just before 1st FC layer

def Hook(grad):
    grad_np = grad.data
    print("Gradient before first FC ")
    print(grad_np)
    ...
    for i in range(32):
        ...
        if i % 10 == 9:
            ...
            for j in range(32):
                ...
                total+= int(correct)
                print("accurate prediction out of 128 batch samples ", correct)
            print("accurate prediction out of 4096 samples ", total)
            print('[%d, %5d] loss: %.3f' % (epoch + 1, i + 1, running_loss / 2000))
            running_loss = 0.0

#Saving the model with method 1, saving entire model
