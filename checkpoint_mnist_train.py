...
import numpy as np 
import pandas as pd 
# Training data from kaggel in form of csv file
...
#Train Test split of traininig data
	...
	for i in range(32):
		...
		if i % 10 == 9:
			...
			for j in range(32):
				...
				total+= int(correct)
				print("accurate prediction of 128 data smaples ", correct)
			print("accurate prediction for 4096 data samples ", total)
			print('[%d, %5d] loss: %.3f' % (epoch + 1, i + 1, running_loss / 2000))
			running_loss = 0.0

	# Instead of state_dict saving the complete model method 1

	torch.save(model, "mnist_model_meth1 " + str(epoch))
